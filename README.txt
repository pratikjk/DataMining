...
Description	: Simple Python implementation of the Apriori Algorithm on Gene Data
...
Usage:
	...
	Eg.
		$ python apriori_frequent_itemset.py -f DATASET.csv -s 0.15 -c 0.6 -t "body has any of G6UP,G82Down,G72UP"


	The program accepts min_support, min_confidence, dataset, and template values through command-line options. 
	The dataset file accepted is only csv files
	If they are not provided, default values are also mentioned in the code. The algorithm runs by first generating frequent items-ets.
	...
	only multiple OR rules are supported at this time for template 3.
